import { z } from "zod";
import { apiTokenSchema, environmentSchema, createBaseSchema, createRetrieveSchema, createListSchema, createDeleteSchema, paginationSchema, fieldTypeSchema } from "../../utils/sharedSchemas.js";
...
export const schemaSchemas = {
    ...
    update_item_type: createBaseSchema().extend({
        ...
        tree: z.boolean().optional()
            .describe("Whether the item type is structured as a tree.")
    }).refine(data => Object.keys(data).length > 2, // apiToken + environment + at least one field
    {
...
`),
    update_field: createBaseSchema().extend({
        ...
        field_type: fieldTypeSchema.optional().describe("The updated type of the field. If switching from a basic text field and no specific editor is required, consider using 'structured_text' instead of 'text' to enable record links and modular blocks. " +
...
export const schemaActionsList = Object.keys(schemaSchemas);
