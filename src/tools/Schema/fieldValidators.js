...
export const lengthValidatorSchema = z.object({
    ...
    }).refine(data => !!(data.min || data.max || data.exact), { message: "At least one of min, max, or exact must be specified" }).describe("Length validation for text fields. Example: { \"min\": 3, \"max\": 100 }")
});
/**
...
export const numberRangeValidatorSchema = z.object({
    ...
    }).refine(data => !!(data.min || data.max), { message: "At least one of min or max must be specified" }).describe("Range validation for number fields. Example: { \"min\": 0, \"max\": 100 }")
});
/**
 * Enum validator
...
export const patternValidatorSchema = z.object({
    ...
    }).refine(data => data.predefined !== "customPattern" || !!data.regex, { message: "When using customPattern, regex must be provided" }).describe("Pattern validator for applying regex or predefined patterns.")
});
/**
 * File size validator
...
export const imageDimensionValidatorSchema = z.object({
    ...
    }).refine(data => !!(data.width || data.height || data.ratio), { message: "At least one dimension constraint must be specified" }).describe("Image dimension validator to ensure proper image sizes and proportions.")
});
/**
...
export const itemsNumberValidatorSchema = z.object({
    ...
    }).refine(data => !!(data.min || data.max || data.exact), { message: "At least one of min, max, or exact must be specified" }).describe("Items number validator to control how many items can be selected.")
});
/**
...
export const singleBlockBlocksValidatorSchema = z.object({
    ...
    }).describe("REQUIRED validator for single_block fields. Example: { \"item_types\": [] }")
});
/**
...
export const dateTimeValidatorSchema = z.object({
    ...
    }).refine(data => !!(data.min || data.max), { message: "At least one of min or max must be specified" }).describe("Date/time range validator to set allowed date boundaries.")
});
/**
 * Video URL validator
 * For video fields
 ...
 */
export function createValidatorsSchema(fieldType) {
 ...
 */
function getExampleValidator(fieldType) {
    switch (fieldType) {
        case 'string':
            return '{ "required": {} }';
        case 'text':
