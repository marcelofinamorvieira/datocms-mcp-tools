...
export const errorMessages = {
    ...
    minLength: (min) => `Must be at least ${min} character${min === 1 ? '' : 's'} long`,
    maxLength: (max) => `Must not exceed ${max} character${max === 1 ? '' : 's'}`,
    pattern: (pattern) => `Must match pattern: ${pattern}`,
    enum: (values) => `Must be one of: ${values.join(', ')}`,
    url: "Must be a valid URL",
    email: "Must be a valid email address",
    uuid: "Must be a valid UUID",
    // Number validation messages
    minValue: (min) => `Must be greater than or equal to ${min}`,
    maxValue: (max) => `Must be less than or equal to ${max}`,
    ...
    requiredKeys: (keys) => `Must include required fields: ${keys.join(', ')}`,
    extraKeys: "Contains unrecognized fields",
    // Array validation messages
    invalidArray: "Must be a valid array",
    minItems: (min) => `Must contain at least ${min} item${min === 1 ? '' : 's'}`,
    maxItems: (max) => `Must contain no more than ${max} item${max === 1 ? '' : 's'}`,
    ...
    invalidValidator: (fieldType) => `Validator not supported for field type '${fieldType}'`,
    invalidAppearance: (fieldType) => `Appearance configuration not valid for field type '${fieldType}'`,
    ...
    maxSize: (size) => `File size must not exceed ${size}`,
    invalidFileType: (types) => `File type must be one of: ${types.join(', ')}`,
    // Image validation messages
    minWidth: (width) => `Image width must be at least ${width}px`,
    maxWidth: (width) => `Image width must not exceed ${width}px`,
    minHeight: (height) => `Image height must be at least ${height}px`,
    maxHeight: (height) => `Image height must not exceed ${height}px`,
    aspectRatio: (ratio) => `Image must have aspect ratio ${ratio}`
};
