...
export const apiTokenSchema = z.string()
    ...
    .describe("DatoCMS API token for authentication. If you are not certain of one, ask for the user, do not hallucinate.");
/**
...
export const environmentSchema = z.string()
    .optional()
    .describe("The name of the DatoCMS environment to interact with. If not provided, the primary environment will be used.");
/**
 ...
 */
export function createIdSchema(entityName) {
    return z.string()
 ...
 */
export function createRetrieveSchema(entityName) {
 ...
 */
export function createDeleteSchema(entityName) {
    return createBaseSchema().extend({
        [`${entityName}Id`]: createIdSchema(entityName)
    });
}
